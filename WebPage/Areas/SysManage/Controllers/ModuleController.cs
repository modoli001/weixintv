    ...
    public class ModuleController : BaseController
    {
        ...
        public ActionResult Delete(string idList)
        {
            ...
            try
            {
                ...
                {
                    var idlist1 = idList.Trim(',').Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(p => p).ToList();
                    //删除模块，同时删除下级模块，相关权限、用户权限、角色权限
                    var bl = this.ModuleManage.Remove(idlist1);
                    if (bl) { json.Status = "y"; }
                }
                else
                {
