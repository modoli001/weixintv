...
namespace WebPage.Areas.SysManage.Controllers
{
    public class PermissionController : BaseController
    {
        ...
        public ActionResult Detail(int? id)
        {
            try
            {
                var _entity = this.PermissionManage.Get(p => p.ID == id) ?? new Domain.SYS_PERMISSION() { SHOWORDER = 0};
        ...
        public ActionResult GetTree()
        {
            ...
            try
            {
                ...
                var result = query.Select(m => new
                {
                    ...
                    text = m.NAME,
                    icon = m.LEVELS == 0 ? "fa fa-circle text-warning" : "fa fa-circle text-navy"
                }).ToList();
        ...
        public ActionResult SaveAllocation()
        {
            ...
            try
            {
                ...
                {
                    if (!this.RolePermissionManage.SetRolePermission(int.Parse(id), perid)) { json.Msg = "保存失败"; WriteLog(Common.Enums.enumOperator.Allocation, "设置角色权限，结果：" + json.Msg, Common.Enums.enumLog4net.ERROR); return Json(json); }
                }
