    ...
    public class PermissionManage : RepositoryBase<Domain.SYS_PERMISSION>, IService.IPermissionManage
    {
        ...
        public List<int> GetPermissionIdBySysId(string sysId)
        {
            try
            {
                ...
                {
                    string sql = "select p.id from sys_permission p where exists(select 1 from sys_module t where 1=@sysid and t.id=p.moduleid)";
                    DbParameter para = new SqlParameter("@sysid", 1);
                    return this.SelectBySql<int>(sql);
                }
